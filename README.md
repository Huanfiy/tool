...
工作室将 Three.js 空间与插画式美术结合：转角木桌由靠墙钢架承重，桌前和中央留空，打印机置于左侧地面，右侧架子摆放三个立体动漫手办。窗扇向外开合，窗景采用动漫风格的奇幻田园：阳光、云层、分层山丘、树木、花草和飞鸟由本地几何与 Canvas 绘制，并随微风动态变化。纸张、木纹和布料纹理由 Canvas 生成，模型由代码构建。显示器使用 CSS3D 与 WebGL 深度遮挡，保留原生点击、输入、文字选择和滚动。静置时以 30 fps 为目标并缓存阴影，操作镜头或运行设备时最高 60 fps；使用电脑时设备继续运行，切换到后台时暂停场景渲染。
...
├── js/studio-window.js   # 双扇开窗与立体自然窗景
